...
def load_latest_batch() -> pd.DataFrame:
    ...
    if not files:
        raise FileNotFoundError("Tidak ada batch streaming yang ditemukan!")
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    """Cleaning dan feature engineering untuk data streaming."""
    before = len(df)
    df = df.dropna()
    df = df.drop_duplicates()
    after = len(df)
    ...
    df["is_sell"]   = df["is_sell"].astype(int)

    df = df.drop(["timestamp", "amount", "quantity", "symbol"], axis=1)
